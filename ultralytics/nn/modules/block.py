...
class SEBlock(nn.Module):
    def __init__(self, c1, reduction=16):
        ...
        self.fc1 = nn.Linear(c1, c1 // reduction, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Linear(c1 // reduction, c1, bias=False)
...
class ChannelAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.sigmoid(avg + mx).view(b, c, 1, 1)
        return x * out
...
class SpatialAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.sigmoid(self.conv(torch.cat([avg, mx], dim=1)))
        return x * out

class CBAMBlock(nn.Module):
    def __init__(self, c1, reduction=16, kernel_size=7):
        super().__init__()
        self.channel_attention = ChannelAttentionBlock(c1, reduction)
...
class DWBlock(nn.Module):
    def __init__(self, c1, k=3, s=1, p=None):
        super().__init__()
        p = autopad(k, p)
...
class GhostModule(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=None, ratio=4, dw_kernel_size=3):
        super().__init__()
        p = autopad(k, p)
        self.primary = nn.Sequential(
            nn.Conv2d(c1, c2 // ratio, k, s, p, bias=False),
            nn.BatchNorm2d(c2 // ratio),
            nn.ReLU(inplace=True)
        )
        self.ghost = nn.Sequential(
            nn.Conv2d(c2 // ratio, c2 - c2 // ratio, dw_kernel_size, 1, autopad(dw_kernel_size), groups=c2 // ratio, bias=False),
            nn.BatchNorm2d(c2 - c2 // ratio),
            nn.ReLU(inplace=True)
        )
